			...
			Else
				...
				If Me.LineDirection = LineDirection.Horizontal Then
					For i = 0 To Convert.ToInt32(panel.Rect.Width / (Me.Dash + Me.Space)) - 1
                        ...
                        dashBrick.Rect = New RectangleF(i * (Me.Dash + Me.Space), Convert.ToSingle(panel.Rect.Height) / 2 - Convert.ToSingle(Me.LineWidth) / 2, Me.dash_Renamed, Me.LineWidth)
                    ...
                    endBrick.Rect = New RectangleF(i * (Me.Dash + Me.Space), Convert.ToSingle(panel.Rect.Height) / 2 - Convert.ToSingle(Me.LineWidth) / 2, panel.Rect.Width - i * (Me.Dash + Me.Space), Me.LineWidth)
				...
				ElseIf Me.LineDirection = LineDirection.Vertical Then
					For i = 0 To Convert.ToInt32(panel.Rect.Height / (Me.Dash + Me.Space)) - 1
						...
						dashBrick.Sides = BorderSide.None
						dashBrick.BackColor = panel.Style.ForeColor
                        dashBrick.Rect = New RectangleF(Convert.ToSingle(panel.Rect.Width) / 2 - Convert.ToSingle(Me.LineWidth) / 2, i * (Me.Dash + Me.Space), Me.LineWidth, Me.Dash)
						panel.Bricks.Add(dashBrick)
					Next i
					Dim endBrick As New LineBrick()
					endBrick.Sides = BorderSide.None
					endBrick.BackColor = panel.Style.ForeColor
                    endBrick.Rect = New RectangleF(Convert.ToSingle(panel.Rect.Width) / 2 - Convert.ToSingle(Me.LineWidth) / 2, i * (Me.Dash + Me.Space), Me.LineWidth, panel.Rect.Height - i * (Me.Dash + Me.Space))
					panel.Bricks.Add(endBrick)
				End If
			End If
		End Sub
